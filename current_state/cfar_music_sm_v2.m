...
r2 = zeros(n_steps, 1);
v2 = zeros(n_steps, 1);
IQ_U = zeros(n_samples, n_steps);
IQ_D = zeros(n_samples, n_steps);
detu = zeros(n_samples, n_steps);
detd = zeros(n_samples, n_steps);
...
for i = 1:n_steps
    ...
    IQ_UP = fftshift(fft(xr_d_up));
    IQ_DOWN = fftshift(fft(xr_d_down));
...
%     IQ_DOWN(90:110) = 0;
    ...
    % scaled detections
    detu(:,i) = up_detections.*abs(IQ_UP);
    detd(:,i) = down_detections.*abs(IQ_DOWN);

    [highest_SNR_up, pk_idx_up]= max(up_detections.*abs(IQ_UP));
    [highest_SNR_down, pk_idx_down] = max(down_detections.*abs(IQ_DOWN));
    ...
    % ROOT MUSIC
    fbu_rng = rootmusic(xr_d_up,1,fs_adc);
    fbd_rng = rootmusic(xr_d_down,1,fs_adc);
   
    fd = -(fbu_rng+fbd_rng)/2;
    r2(i) = beat2range([fbu_rng fbd_rng],sweep_slope,c);
    v2(i) = dop2speed(fd,lambda)/2;
    fbu2(i) = fbu_rng;
    fbd2(i) = fbd_rng;
end

%% Verify CFAR visually
close all
figure 
tiledlayout(2,1)
nexttile
...
stem(t, range_array,'DisplayName','CFAR')
title("Range")
ylabel("range (m)")
xlabel("time (s)")
hold on
stem(t, expected_range, 'DisplayName','Expected')
hold on
stem(t, r2, 'DisplayName','ROOTMUSIC')
legend
nexttile
stem(t, speed_array, 'DisplayName','CFAR')
title("Speed")
ylabel("speed (m/s)")
xlabel("time (s)")
hold on
stem(t, expected_speed, 'DisplayName','Expected')
hold on
stem(t, v2, 'DisplayName','ROOTMUSIC')
legend
