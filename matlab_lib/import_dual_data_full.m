function [fc, c, lambda, tm, bw, k, rpi_iq_u, rpi_iq_d,...
    usb_iq_u, usb_iq_d, t_stamps] = import_dual_data_full(lhs_rad, rhs_rad)
    % Parameters
    fc = 24.005e9;
...
%     size(lhs_rad_tbl)
%     size(rhs_rad_tbl)
    lhs_i_up = table2array(lhs_rad_tbl(:,1:200));
    lhs_i_dn = table2array(lhs_rad_tbl(:,201:400));
    lhs_q_up = table2array(lhs_rad_tbl(:,401:600));
    lhs_q_dn = table2array(lhs_rad_tbl(:,601:800));

    usb_i_up = table2array(rhs_rad_tbl(:,1:200));
    usb_i_dn = table2array(rhs_rad_tbl(:,201:400));
    usb_q_up = table2array(rhs_rad_tbl(:,401:600));
    usb_q_dn = table2array(rhs_rad_tbl(:,601:800));
