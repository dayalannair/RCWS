...
%% PLOT BOTH SIDES VIDEO AND RAW DATA + THRESHOLD + GATE
% for i = 1:loop_count
% 
...
%% PLOT LEFT SIDES VIDEO AND RAW DATA + THRESHOLD + GATE
% close all
vid_lhs.CurrentTime = 0;
vid_rhs.CurrentTime = 0;
...
for i = 1:loop_count
    ...
    lambda, k, c, dnDets1(i,:), upDets1(i,:), nbins, n_fft, ...
    f_pos, scan_width, calib, lhs_road_width, beat_count_in1);
    % Raw data: 3xradar frames for 1x video frame
    disp(['Sweep: ',num2str(i)])
    if hold_frame == 2
        vidFrame = readFrame(vid_lhs);
        set(v1,'CData' ,vidFrame);
